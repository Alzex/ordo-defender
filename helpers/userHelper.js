...
const userHelper = {
    async mute(tg, chat, violator, issuer, duration, langCode, reason) {
        ...
        await punishmentManagers.addPunishment(violator.id, issuer.id, chat.id, reason ? reason : null, enums.PUNISHMENT.MUTE);
    ...
    async kick(tg, violator, issuer, chat, langCode, reason) {
        ...
        await punishmentManagers.addPunishment(violator.id, issuer.id, chat.id, reason, enums.PUNISHMENT.KICK).catch((e) => {
    ...
    async ban(tg, violator, issuer, chat, langCode, reason) {
        ...
        await tg.banChatMember(chat.id, violator.id, { revoke_messages: true }).catch((e) => {
            logger.tg.fatal(e.message);
            throw e;
        });

        await punishmentManagers.addPunishment(violator.id, issuer.id, chat.id, null, enums.PUNISHMENT.BAN).catch((e) => {
            logger.db.error(e.message);
        });
    ...
    async warn(tg, violator, issuer, chat, langCode, reason) {
        ...
        await punishmentManagers.addPunishment(violator.id, issuer.id, chat.id, reason).catch((e) => {
            logger.db.fatal(e.message);
            throw e;
        });

        const punishments = await punishmentManagers.getValidUsersPunishmentsFromChat(violator.id, chat.id).catch((e) => {
            logger.db.fatal(e.message);
            throw e;
        });
