...
const moderationController = {
  ...
  unwarn: async (ctx) => {
    ...
    const result = await punishmentManagers
      .disposeEarliestPunishment(violator.id, ctx.chat.id, ctx.from.id)
      .catch((e) => {
  ...
  mute: async (ctx) => {
    ...
    if (argumentsQuery[1]) {
      reason = ctx.message.text.substring(
        argumentsQuery[0].length + argumentsQuery[1].length + 2,
      );
    }
  ...
  history: async (ctx) => {
    ...
    const amount = await punishmentManagers.countAllPunishments(
      ctx.from.id,
      ctx.chat.id,
      forChat,
    );
    ...
    if (amount === 0) {
      ...
      await ctx
        .editMessageText(text, {
          parse_mode: 'HTML',
          ...keyboards.backKeyboard(ctx.state.langCode, ctx.from.id),
        })
        .catch((e) => {
          logger.tg.fatal(e.message);
          throw e;
      ...
      punishments = await punishmentManagers
        .getPunishments(ctx.from.id, ctx.chat.id, 0, 5, forChat)
        .catch((e) => {
          logger.db.fatal(e.message);
          throw e;
        });
      ...
      punishments = await punishmentManagers
        .getPunishments(ctx.from.id, ctx.chat.id, curPage * 5, 5, forChat)
        .catch((e) => {
          logger.db.fatal(e.message);
          throw e;
        });
      markup = keyboards.pagesKeyboard(
        curPage === 0,
        curPage + 1 === maxPages,
        curPage,
        ctx.from.id,
    ...
    for (const punishment of punishments) {
      ...
      const issuer = await ctx.telegram
        .getChatMember(ctx.chat.id, Number(punishment.issuer_id))
        .catch((e) => {
          console.error(
            `[TG API ERROR] moderationController history  ctx.telegram.getChatMember:`,
            e.message,
          );
        })
        .catch((e) => {
      ...
      if (forChat) {
        ...
        const violator = await ctx.telegram
          .getChatMember(ctx.chat.id, Number(punishment.violator_id))
          .catch((e) => {
            if (e.error_code === 400) {
              text = text.replaceAll('{user}', '<b>DELETED</b>');
              return;
      ...
      text = text.replace(
        '{date}',
        moment(punishment.issued_at).format('DD.MM.YYYY HH:mm:ss'),
      );
      let valid =
        punishment.disposed_at && punishment.disposed_by
          ? tr.entities.warnDisposed
          : tr.entities.warnExpired;
      valid = punishment.disposed_at ? valid : tr.entities.warnValid;
      const reason = punishment.reason
        ? punishment.reason
        : tr.reasonNotSpecified;
