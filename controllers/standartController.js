...
const standartController = {
  ...
  profile: async (ctx) => {
    ...
    const punishments = await punishmentManager
      .getValidUsersPunishmentsFromChat(ctx.from.id, ctx.chat.id)
      .catch((e) => {
        logger.db.fatal(e.message);
        throw e;
      });
    ...
    if (ctx.update.callback_query) {
      await ctx
        .editMessageText(text, {
          parse_mode: 'HTML',
          ...keyboards.profileKeyboard(ctx.state.langCode, ctx.from.id),
        })
        .catch((e) => {
          logger.tg.fatal(e.message);
          throw e;
  ...
  async settings(ctx) {
    ...
    if (!ctx.update.callback_query) {
      await ctx
        .reply(langs[ctx.state.langCode], {
          parse_mode: 'HTML',
          ...keyboards.settingsKeyboard(ctx.from.id),
        })
        .catch((e) => {
          logger.tg.fatal(e.message);
          throw e;
    ...
    await ctx
      .editMessageText(langs[ctx.state.langCode], {
        parse_mode: 'HTML',
        ...keyboards.settingsKeyboard(ctx.from.id),
      })
      .catch((e) => {
